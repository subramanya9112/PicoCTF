msg = """Mjbjhfly Ujcbeyz eblgj, rxpm e cbenj eyz gpepjui exb, eyz kblhcmp dj pmj kjjpuj
...
for letter in msg:
    if ord('a') <= ord(letter) <= ord('z'):
        print(chr(key.index(chr(ord(letter) - ord('a') + ord('A'))) + ord('a')), end='')
    elif ord('A') <= ord(letter) <= ord('Z'):
        print(chr(key.index(letter) + ord('A')), end='')
    else:
        print(letter, end="")
